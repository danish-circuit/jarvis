import { Settings, ArrowUpCircle, Bug, AlertTriangle, CalendarClock } from 'lucide-react';
...
export default function SidebarFooter({
  ...
  const scheduledTasksEnabled = plugins.some((p) => p.name === 'scheduled-tasks' && p.enabled);
    ...
    <div className="flex-shrink-0" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0)' }}>
      ...
      {scheduledTasksEnabled && (
        <div className="hidden px-2 pt-1.5 md:block">
          ...
          >
            ...
            <span className="text-sm">{t('actions.scheduledTasks')}</span>
          </button>
        </div>
      )}
      ...
      {!IS_PLATFORM && (
        <div className="hidden px-3 py-2 text-center md:block">
          ...
          >
            Jarvis v{currentVersion} – {t('branding.openSource')}
          </a>
        </div>
      )}
      ...
      {scheduledTasksEnabled && (
        <div className="px-3 pt-2 md:hidden">
          ...
          >
            ...
            <span className="text-sm font-normal text-foreground">{t('actions.scheduledTasks')}</span>
          </button>
        </div>
      )}
