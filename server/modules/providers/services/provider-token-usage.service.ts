...
/**
 * Sums token usage for one Pi session transcript.
 *
 * Pi stamps per-response usage onto each assistant message instead of keeping a
 * session running total, so the total is the sum over the active branch. The
 * branch walk is reused from the Pi facet rather than duplicated — Pi sessions
 * are a tree, so summing every line in the file would double-count abandoned
 * `/fork` branches.
 */
function readPiTokenUsage(fileContent: string): TokenUsageResult {
  const parsed = parsePiSessionContent('', fileContent);
  let inputTokens = 0;
  let outputTokens = 0;

  for (const entry of buildPiActiveBranch(parsed.entries)) {
  ...
  return {
    ...
    async getSessionTokenUsage(sessionId: string): Promise<TokenUsageResult> {
      ...
      if (session.provider === 'pi') {
        ...
        return readPiTokenUsage(await dependencies.readTextFile(session.jsonl_path));
      }
