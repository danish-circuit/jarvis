...
import { appendFilesInputTag, buildPiImageContents } from '@/shared/image-attachments.js';
 ...
 * protocol has no approval message at all. So "plan" cannot be a mode toggle
...
async function spawnPi(command, options = {}, ws, context) {
  ...
  // a plain error rather than a half-sent prompt. Unlike the other CLIs, Pi
  // takes images as native base64 content blocks on its RPC `prompt` command,
  // so there is no <images_input> path tag to append or strip back out.
  const imageContents = await buildPiImageContents(images, workingDir);
  // General file attachments have no native slot, so they ride along as the
  // shared <files_input> path list that every history adapter strips for display.
  const promptWithFiles = appendFilesInputTag(command?.trim() || '', files);
